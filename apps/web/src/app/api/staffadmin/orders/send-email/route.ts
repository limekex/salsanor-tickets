...
export async function POST(request: NextRequest) {
    try {
        ...
        // Fetch order with all necessary relations - ensure it belongs to the org
        const order = await prisma.order.findUnique({
            where: { 
                id: orderId,
                organizerId, // Security: ensure org admin can only send emails for their org
            },
            include: {
                PersonProfile: {
                    include: {
                ...
                if (order.Registration.length > 0 && order.CoursePeriod) {
                    ...
                    result = await emailService.sendTransactional({
                        ...
                        variables: {
                            ...
                            courseTime: 'See detailed schedule',
                            courseLocation: order.Organizer.city || 'See detailed information',
                ...
                if (order.EventRegistration.length > 0) {
                    ...
                    result = await emailService.sendTransactional({
                        ...
                        variables: {
                            ...
                            eventLocation: order.EventRegistration[0].Event.locationName || 'Not specified',
                            ticketNumber: order.orderNumber,
                            ticketUrl: `${process.env.NEXT_PUBLIC_APP_URL}/my/tickets`,
                            organizerName: order.Organizer.name,
                            currentYear: new Date().getFullYear().toString()
                ...
                result = await emailService.sendTransactional({
                    ...
                    variables: {
                        ...
                        eventName: order.Registration.length > 0 
                            ...
                            : order.EventRegistration[0]?.Event.title || 'Order',
                ...
                result = await emailService.sendTransactional({
                    ...
                    variables: {
                        ...
                        refundMessage: order.status === 'REFUNDED' 
                            ? 'The refund will be credited to your account within 5-10 business days.'
                            : 'Refund is being processed manually.',
        ...
        console.error('[staffadmin-send-order-email] Error:', error)
